export interface WeatherInfo {
  ...
  range: string;
  status: string;
  stats: Array<{ icon: string; label: string; value: string; sub: string; progress?: number }>;
  forecast: Array<{ day: string; icon: string; min: number; max: number }>;
  safety: Array<{ icon: string; text: string }>;
}

export async function getWeather(city: 'krabi' | 'lanta'): Promise<WeatherInfo> {
  // Returns mock weather info matching the previous screens exactly
  const krabiData: WeatherInfo = {
    temp: "31°",
    condition: "晴朗",
    range: "最高 33° / 最低 27°",
    status: "适宜游玩 (绿旗)",
  ...
  return city === 'krabi' ? krabiData : lantaData;
}
